...
doc = FreeCAD.openDocument(os.path.join(HERE, "part22_sheetmetal.FCStd"))
...
COLOR = (60, 110, 70)  # matches the machine's actual green paint, per lathe_isosvg.py
...
out.append('<text x="20" y="24" font-size="16" font-weight="bold">'
            'Part #22 bracket -- FreeCAD SheetMetal reconstruction, PHOTO-EST (not measured)</text>')
for _, pts, fill in faces:
    d = " ".join(("%.1f,%.1f" % tx(p)) for p in pts)
    out.append(f'<polygon points="{d}" fill="rgb{fill}"/>')
out.append('</svg>')
svg = os.path.join(HERE, "part22_sheetmetal_iso.svg")
open(svg, "w").write("\n".join(out))
